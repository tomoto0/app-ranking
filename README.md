...
サーバー内の`node-cron`を使用して、毎日JST 7:00（UTC 22:00）に自動でランキングデータを取得し、履歴データとして蓄積します。サーバー起動時に自動的にcronジョブがスケジュールされ、外部タスクの起動なしにバックグラウンドで実行されます。ネットワークエラー時には自動リトライ機能（最大3回）が働きます。
